fetch("http://localhost:3000/playlist")
    ...
    .then((json) => {
        if (json.length === 0) {
            document.getElementById('spotify-playlists').innerHTML = `<p style="color: whitesmoke;">Belum ada Lagu</p>`;
        } else {
...
function getMostPlayedSong() {
    ...
    fetch("http://localhost:3000/countedPlaylist")
        ...
        .then((json) => {
            dataFetched = true;
            if (json.length === 0) {
                document.getElementById('mostPlayedSongs').innerHTML = `<p style="color: whitesmoke;">Belum ada Lagu sering didengar</p>`;
            } else {
                for (let index = 0; index < json.length; index++) {
